...
from src.torch_src.torch_lstm import LSTMRegressor
from src.torch_src.torch_saliency import SaliencyHandler
from src.torch_src.torch_utils import to_tensor
from src.torch_src.sequence_dataset import SequenceDataset


class SklearnLightningSequenceRegressor(BaseEstimator, RegressorMixin, torch.nn.Module):
    def __init__(self, feature_names, out_classes, logger, callbacks, ensemble_k=0, batch_size=32,
    ...
    def _create_model(self, ensemble_k, ensemble_prior, ensemble_bootstrap, *model_args, **model_kwargs):
        if ensemble_k > 1:
            self.model_init_args = (LSTMRegressor, ensemble_k, *model_args)
            ...
            self.model_class = LSTMRegressor
    ...
    def create_loader(self, inputs, targets, batch_size=None, train=False):        
        dataset = SequenceDataset(inputs, targets, train=train)
